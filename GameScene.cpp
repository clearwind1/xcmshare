...
#include "ResetAccountScene.h"
#include "SetMachineIDScene.h"
#include "ActivationCodeScene.h"
...
void GameScene::shotlight()
{
    ...
    m_pshotlight->runAction(Sequence::create((ActionInterval*)m_prunlightaction,m_pshotlightaction,callback, NULL));
 ...
 * 版本命令Handler
 */
void GameScene::versionInfoHandler()
{
#if USE_EVENT_LISTENER == 0
	m_VersionInfo = SlaveMachine::getInstance()->getVersionInfo();
#endif

	if (m_VersionInfo.isNew == true) {
		log("TWinDebug: GameScene versionInfoHandler()");
		UserDefault::getInstance()->setIntegerForKey(VERSION_NUMBER_KEY, m_VersionInfo.versionNumber);
		auto sc = LogoLayer::createScene();
		Director::getInstance()->replaceScene(sc);

		m_VersionInfo.isNew = false;
#if USE_EVENT_LISTENER == 0
		SlaveMachine::getInstance()->setVersionInfo(m_VersionInfo);
#endif
	}
}
 ...
 * 換場景命令Handler
 */
void GameScene::sceneInfoHandler()
{
#if USE_EVENT_LISTENER == 0
	m_SceneInfo = SlaveMachine::getInstance()->getSceneInfo();
#endif

	if (m_SceneInfo.isNew == true) {
		log("TWinDebug: GameScene sceneInfoHandler() Scene: %d", m_SceneInfo.sceneID);

		Scene* sc;
		if (m_SceneInfo.sceneID == 1) {				// 切換到設定機台ID場景
			sc = SetMachineIDLayer::createScene();
			Director::getInstance()->pushScene(sc);
		}
		if (m_SceneInfo.sceneID == 2) {				// 切換到設定場景
			sc = SettingLayer::createScene();
			Director::getInstance()->pushScene(sc);
		}
		else if (m_SceneInfo.sceneID == 3) {			// 切換到歸零場景
			sc = ResetAccountLayer::createScene();
			Director::getInstance()->pushScene(sc);
		}
		else if (m_SceneInfo.sceneID == 5) {			// 切換到打碼場景
			sc = ActivationCodeLayer::createScene();
			Director::getInstance()->pushScene(sc);
		}
		else if (m_SceneInfo.sceneID == 6) {			// 切換到查帳場景
			sc = AccountLayer::createScene();
			Director::getInstance()->pushScene(sc);
		}

		m_SceneInfo.isNew = false;

#if USE_EVENT_LISTENER == 0
		SlaveMachine::getInstance()->setSceneInfo(m_SceneInfo);
#endif
	}
}
...
{
	BaseLayer::update(dt);		// Steve - 在收到特定命令後進入後台或是回到資源加載場景
